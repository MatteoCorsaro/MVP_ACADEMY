...
public class ReservationDao {
    ...
    public void update(Reservation reservation,String newName){
        Statement statement;
        try{
            String query=String.format("update %s set state='%s' where state ='%s' and data='%s'",TABLE,newName,RESERVATION_STATE.NON_ACCETTATA.toString(),reservation.getReservationDate().format(DateTimeFormatter.ofPattern(FORMATT)));
            Connection conn = Singleton.getLoginInstance().getDaoFactory().getConnection();
            statement=conn.createStatement();
            statement.executeUpdate(query);
            statement.close();
